...
split_baseR <- sample(nrow(anes_short), 
                      size = 0.8*nrow(anes_short))  

train_baseR <- anes_short[split_baseR, ]
test_baseR <- anes_short[-split_baseR, ]
train_baseR
...
model <- lm(ftobama ~ faminc + pid3 + birthyr + gender, data = train_baseR)
summary(model)
...
#mse(train_baseR, anes$ftobama, estimate, na_rm = TRUE, ...)
mse(train_baseR, train_baseR$ftobama, model, na_rm = TRUE)
