...
logit %>% 
  ...
  ggplot() +
  geom_bar(alpha = 0.8, aes(correct, 
                            fill = correct)) + 
  labs(x = "Correct?",
       y = "Count",
       fill = "Correctly\nClassified") +
...
# create confusion matrix based on predictions from the model with conf_mat() from yardstick
final_mod %>% 
  collect_predictions() %>% 
...
```{r}
# select some features and clean: party, and all FTs
anes_new <- anes %>% 
  select(pid3, starts_with("ft")) %>% 
  mutate(democrat = as.factor(ifelse(pid3 == 1, 1, 0)),
         ftobama = replace(ftobama, ftobama > 100, NA),
...
logit_plot <- final_mod %>% 
  ...
  geom_bar(aes(x = .pred_class, 
               fill = democrat)) +
  facet_wrap(~ democrat) +
  labs(title = "From Logit Fit across All FTs",
       x = "Predicted Party Affiliations", 
       fill = "Democrat",
       caption = "Note: facets are ground truth") +
  theme_minimal()
