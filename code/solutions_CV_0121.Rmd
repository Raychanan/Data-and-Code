...
# function to train the model (training set) and evaluate the model (validation set), across each polynomial regression fit
...
1. Use 10-fold cross validation to build and evaluate a logistic regression predicting `Survived` as a function of interacting `Age` and `Sex`. To answer this, you will need to:
  a. Build the classifer on the training set(s) across folds
  b. Evaluate each classifer using the test set(s) across folds
